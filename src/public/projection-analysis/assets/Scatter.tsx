...
export function Scatter({
  ...
  const circles = useMemo(() => {
    ...
    return data.map((d, i) => {
      ...
      return <circle key={i} opacity={hidePoints ? 0 : brushedSet && !brushedSet.has(d[params.ids]) ? 0.3 : 1} r={2} fill={brushedSet && !brushedSet.has(d[params.ids]) ? 'lightgray' : params.category ? colorScale(d[params.category]) : 'lime'} cx={xVal + 0.8} cy={yScale(d[params.y]) + -2.5} stroke='black' onMouseOver={e => handleMouseOver(e, d[params.category])} onMouseOut={handleMouseOut} />;
    });
